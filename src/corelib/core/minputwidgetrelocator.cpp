...
void MInputWidgetRelocator::update()
{
    ...
    if (inputWidget
        ...
        MScrollChain *newChain = 0;
        if (oldChain && oldChain->isValidForTarget(inputWidget)) {
            // Use the same chain so we get current scrolling state.
            newChain = oldChain;
        } else {
            newChain = new MScrollChain(inputWidget, rootElement);

            if (oldChain) {
                if (oldChain->isScrolling()) {
                    oldChain->stopScrolling();
                }
        ...
        if (microRect.isValid() && newChain->count() > 0) {
            ...
            // and an anchor point.
            if (!widgetWasDocked) {
                if (needsRelocation(newChain, inputWidget, microRect)) {
...
bool MInputWidgetRelocator::needsRelocation(const MScrollChain *chain,
                                            ...
                                            const QRect &localRect)
{
    ...
    // If scrolling is ongoing don't check for visibility other than that the target rect
    // is going to be within exposedContentRect(). Checking clipping areas of moving viewports is
    // difficult and we don't currently need it because viewport scrollers make sure that
    // cursor is not clipped.
    if (chain->isScrolling()) {
        return !exposedContentRect().contains(rect.translated(chain->scrollingAmountLeft()));
    } else {
