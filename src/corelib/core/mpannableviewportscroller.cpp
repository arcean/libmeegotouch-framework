                                               ...
                                               int duration, const QEasingCurve &easingCurve)
{
    ...
    if (contentsOffset.isNull()) {
        emit scrollingFinished(widget);
    } else {
        // Connect to private member since the signal is not otherwise accessible.
        const QPropertyAnimation &animation = viewport->d_func()->scrollToAnimation;

        connect(&animation, SIGNAL(finished()),
                this, SLOT(handleScrollAnimationFinished()),
                Qt::UniqueConnection);
...
void MPannableViewportScroller::handleScrollAnimationFinished()
{
    QPropertyAnimation *animation = qobject_cast<QPropertyAnimation *>(sender());
    Q_ASSERT(animation);
    QGraphicsWidget *widget = qobject_cast<QGraphicsWidget *>(animation ? animation->targetObject() : 0);
    Q_ASSERT(widget);

    disconnect(animation, 0, this, 0);
    emit scrollingFinished(widget);
}
