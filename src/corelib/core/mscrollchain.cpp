...
{
    initialOffsetFromRoot = currentOffsetFromRoot();
    totalScrollAmount = QPoint();
    for (ChainItemList::iterator item = chainItems.begin(); item != chainItems.end(); ++item) {
        totalScrollAmount += item->contentsTranslation;
        item->isBeingScrolled = true;
...
void MScrollChain::buildScrollChainItems()
{
    ...
    while (widget
        ...
        if (scroller) {
            ...
            connect(scroller.data(),
                    SIGNAL(scrollingFinished(const QGraphicsWidget*)),
                    SLOT(handleScrollingFinishedForWidget(const QGraphicsWidget *)),
                    Qt::UniqueConnection);
        }
...
bool MScrollChain::isScrolling() const
{
    bool scrollingOngoing = false;
    for (ChainItemList::const_iterator item = chainItems.begin(); item != chainItems.end(); ++item) {
        if (item->isBeingScrolled) {
            scrollingOngoing = true;
            break;
        }
    }
    return scrollingOngoing;
}

QPoint MScrollChain::currentOffsetFromRoot() const
{
    return rootItem->mapFromItem(scrollTarget, QPointF()).toPoint();
}

QPoint MScrollChain::scrollingAmountLeft() const
{
    QPoint scrollingLeft;
    if (isScrolling()) {
        QPoint scrolled = currentOffsetFromRoot() - initialOffsetFromRoot;
        scrollingLeft = totalScrollAmount - scrolled;
    }
    return scrollingLeft;
}

bool MScrollChain::isValidForTarget(const QGraphicsItem *target) const
{
    if (scrollTarget != target) {
        return false;
    }

    QGraphicsWidget *scrollCandidate = target->parentWidget();
    for (ChainItemList::const_iterator item = chainItems.begin(); item != chainItems.end(); ++item) {
        while (scrollCandidate != item->widget) {
            if (!scrollCandidate) {
                return false;
            }
            scrollCandidate = scrollCandidate->parentWidget();
        }
    }
    return true;
}

void MScrollChain::handleScrollingFinishedForWidget(const QGraphicsWidget *widget)
{
    for (ChainItemList::iterator item = chainItems.begin(); item != chainItems.end(); ++item) {
        if (item->widget == widget) {
            item->isBeingScrolled = false;
            break;
        }
    }
}
