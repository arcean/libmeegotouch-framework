...
#ifdef OLOCK
MGConfItem *MWindowPrivate::OrientationLockedWindowsByName = NULL;
MGConfItem *MWindowPrivate::OrientationLockedWindowsByClass = NULL;
#endif
...
#ifdef OLOCK
bool MWindowPrivate::isOrientationLocked(bool defaultValue) const
{
    bool isLocked = defaultValue;

    Display *display = QX11Info::display();
    WId window = display != NULL ? robustEffectiveWinId() : 0;

    if (window != 0) {
        char *wmName;
        XFetchName(display, window, &wmName);
        QString windowName(wmName);
        XFree(wmName);

        XClassHint classHint;
        XGetClassHint(display, window, &classHint);

        if (OrientationLockedWindowsByName == NULL) {
            OrientationLockedWindowsByName = new MGConfItem("/meegotouch/orientationlock/names");
        }
        if (OrientationLockedWindowsByClass == NULL) {
            OrientationLockedWindowsByClass = new MGConfItem("/meegotouch/orientationlock/classes");
        }

        QStringList byName = OrientationLockedWindowsByName->value().toStringList();
        QStringList byClass = OrientationLockedWindowsByClass->value().toStringList();
        isLocked = byName.contains(windowName) || byClass.contains(classHint.res_name) || byClass.contains(classHint.res_class);
    }
...
bool MWindow::isOrientationAngleLocked() const
{
    ...
    if (d->orientationAngleLocked)
        return d->isOrientationLocked(d->orientationAngleLocked);
    else
        return false;
}
#else
