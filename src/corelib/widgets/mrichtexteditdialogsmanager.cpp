...
void MRichTextEditDialogsManager::rejectTextStylingDialog()
{
    if (dialogs.textStyles.first && dialogs.textStyles.second) {
        dialogs.textStyles.first->reject();
        dialogs.textStyles.second = false;
    }

    if (fontFamilyCombo)
        fontFamilyCombo->dismiss();

    if (fontSizeCombo)
        fontSizeCombo->dismiss();

    if (fontColorCombo)
        fontColorCombo->dismiss();
}
