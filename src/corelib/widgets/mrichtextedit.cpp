...
MRichTextEditPrivate::~MRichTextEditPrivate()
{
    if (textStylingDialogShown)
        MRichTextEditDialogsManager::instance()->rejectTextStylingDialog();
}
...
void MRichTextEditPrivate::_q_showTextStylingOptions()
{
    ...
    textStylingDialogShown = true;

    //guard pointer because dialog can return after this object is destroyed
    QWeakPointer<MRichTextEdit> qPtr(q);
